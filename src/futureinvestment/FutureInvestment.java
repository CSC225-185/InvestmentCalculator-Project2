...
public class FutureInvestment extends Application {
    ...
    private void calculateLoanPayment() {
        ...
        Investment investment = new Investment(interest, year, loanAmount);			//  Create new object using information read from GUI

        //Display monthly payment and total payment
        tfMonthlyPayment.setText(String.format("$%.2f", investment.getMonthlyPayment())); 	// Get the monthly payment from the object, format it to look good and display it in the textbox
        tfTotalPayment.setText(String.format("$%.2f", investment.getTotalPayment())); 	// Get the total payment from the object, format it to look good and display it in the textbox
    }
